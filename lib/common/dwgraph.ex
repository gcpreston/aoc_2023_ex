defmodule Common.DWGraph do
  ...
  def add_edge(graph, from, to, weight) do
    from_adj = Map.get(graph.adjacencies, from)
    new_from_adj = [to | from_adj]
    new_adj = Map.put(graph.adjacencies, from, new_from_adj)
    new_weights = Map.put(graph.weights, {from, to}, weight)

    %__MODULE__{adjacencies: new_adj, weights: new_weights}
  end
end

defimpl Common.Graph, for: Common.DWGraph do
